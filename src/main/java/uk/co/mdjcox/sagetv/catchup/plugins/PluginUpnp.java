...
public class PluginUpnp implements PluginInterface {
    ...
    public Collection<Source> getSources() {
        ...
        items = upnpUtils.findItems(progressTracker, UpnpUtils.ContentType.VIDEO, excludes, services);
    ...
    public Collection<Programme> getProgrammes(Source source, AtomicBoolean stopFlag) {
        boolean isProgrammeless = context.getProperties().getBoolean(source.getSourceId() + ".programmeless", false);
        ...
        for (UpnpItem item : items) {
            ...
            if (isProgrammeless) {
                ...
                URL iconUrl = normaliseURI(iconUri);
                iconUrlString = iconUrl == null ? null : iconUrl.toString();
            } else {
                ...
                title = parentContainer.getTitle();
